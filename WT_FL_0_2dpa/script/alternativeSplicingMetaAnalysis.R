...
MXE_WT_cycle_genes <- rMATs_merge_MXE_WT_META2d %>% filter(meta2d_BH.Q < 0.01) %>% pull(CycID)
...
MXE_FL_cycle_genes <- rMATs_merge_MXE_FL_META2d %>% filter(meta2d_BH.Q < 0.01) %>% pull(CycID)
...
RI_WT_cycle_genes <- rMATs_merge_RI_WT_META2d %>% filter(meta2d_BH.Q < 0.01) %>% pull(CycID)
...
RI_FL_cycle_genes <- rMATs_merge_RI_FL_META2d %>% filter(meta2d_BH.Q < 0.01) %>% pull(CycID)
...
SE_WT_cycle_genes <- rMATs_merge_SE_WT_META2d %>% filter(meta2d_BH.Q < 0.01) %>% pull(CycID)
...
SE_FL_cycle_genes <- rMATs_merge_SE_FL_META2d %>% filter(meta2d_BH.Q < 0.01) %>% pull(CycID)
