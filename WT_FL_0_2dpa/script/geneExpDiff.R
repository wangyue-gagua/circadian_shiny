...
first_count_matrix %>% summarise(across(where(is.numeric), ~ sum(.x > 3))) %>% as.numeric() %>% sd()
second_count_matrix %>% summarise(across(where(is.numeric), ~ sum(.x > 3))) %>% as.numeric() %>% mean()
